...
module.exports = {
	...
	execute: async ({ message, args }, ...rest): Promise<void> => {
		const commands = rest[0];
		if (args[0]) {
			const infoRequest = args[0];
			if (commands.has(infoRequest)) {
				const command = commands.get(infoRequest);
				if (({}).hasOwnProperty.call(command, 'info')) command.info({ 'message': message, 'args': args });
			...
			if (/Dwarf|Elf|Halfling|Human/i.test(infoRequest)) {
				...
				if (race.extra_abilities) race.extra_abilities.forEach(ability => abilities += `${ability.name}: ${ability.description}\n`);
